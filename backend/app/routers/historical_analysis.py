...
from app.services.historical_analysis_v2_2 import (
...
@router.post("", response_model=HistoricalAnalysisResponseV22)
def analyze_history(
...
@router.get("/latest", response_model=HistoricalAnalysisResponseV22)
def latest_analysis(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> HistoricalAnalysisResponseV22:
